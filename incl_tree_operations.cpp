...
// Zawsze U¿ywaæ Middle_of_a_Circle a nie get_position
class Operations
{    
public:
    // mo¿e znajdê use
    ...
    {
        Vector2f distance_c = Middle_of_a_circle(distance_circle);
        Vector2f other = Middle_of_a_circle(ca);
    ...
    {
        Vector2f v_1 = Middle_of_a_circle(c_1);
        Vector2f v_2 = Middle_of_a_circle(c_2);        
...
class Recalculating : public Mesh
{
public:
    // Recalculating Lines
    void Recalculat_lines_VertexArray_based_on_existing_circle_list(list<CircleShape*>& circle_list, VertexArray* Vertex_tab, PrimitiveType type, bool connecting)
    {
    ...
    void Recalculate_set_text_distance_circle(list<Text>& Ltext, list<CircleShape*> Lcircles, CircleShape* p_distance_calculation_circle, Font& f)
    {
        Ltext.clear();
        int i = 1;
    ...
    void Recalculate_closest_neightbour(list<CircleShape*> l, CircleShape* p_distance_calculation_circle)
    {
...
class Triangulation : public Operations
{
    // Big Traingle
    Vector2f left;
    Vector2f right;
...
public:
    ...
    void Create_Super_Triangle(list<CircleShape*> lpoints)
    {
        up = Mesh_up(lpoints); up.y -= 1000;
        left = Down_left(lpoints); left.y += 1000; left.x -= 1000;
        right = Down_right(lpoints); right.y += 1000; right.x += 1000;
    }
